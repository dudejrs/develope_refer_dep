...
from neuronlp2.nn import TreeCRF, VarMaskedGRU, VarMaskedRNN, VarMaskedLSTM, VarMaskedFastLSTM
from neuronlp2.nn import SkipConnectFastLSTM, SkipConnectGRU, SkipConnectLSTM, SkipConnectRNN
...
class StackPtrNetKr(StackPtrNet):
    def __init__(self, word_dim, num_words, morph_dim, num_morphs, syll_dim, num_sylls,
                 morph_tag_dim, num_morph_tags, char_dim, num_chars, pos_dim, num_pos,
                 num_filters, kernel_size, rnn_mode, input_size_decoder, hidden_size,
                 encoder_layers, decoder_layers, num_labels, arc_space, type_space,
                 embedd_word=None, embedd_morph=None, embedd_syll=None, embedd_pos=None, p_in=0.33, p_out=0.33, p_rnn=(0.33, 0.33),
                 biaffine=True, pos=True, morph=True, syll=True, morph_tag=True, char=True,
                 prior_order='inside_out', skipConnect=False, grandPar=False, sibling=False,
                 morph_num_filters=None, morph_kernel_size=None, syll_num_filters=None, syll_kernel_size=None,
                 morph_tag_num_filters=None, morph_tag_kernel_size=None, char_num_filters=None, char_kernel_size=None,
                 morph_cooccur=None, morph_freq=None, syll_cooccur=None, syll_freq=None):

        super(StackPtrNetKr, self).__init__(word_dim, num_words, char_dim, num_chars, pos_dim, num_pos,
                  num_filters, kernel_size, rnn_mode, input_size_decoder, hidden_size,
                  encoder_layers, decoder_layers, num_labels, arc_space, type_space, embedd_word=embedd_word, embedd_pos=embedd_pos, p_in=p_in, p_out=p_out, p_rnn=p_rnn,
                 biaffine=biaffine, pos=pos, char=char, prior_order=prior_order, skipConnect=skipConnect, grandPar=grandPar, sibling=sibling)

        self.word_embedd = Embedding(num_words, word_dim, init_embedding=embedd_word)
        self.morph_embedd = Embedding(num_morphs, morph_dim, init_embedding=embedd_morph) if morph else None
        self.syll_embedd = Embedding(num_sylls, syll_dim, init_embedding=embedd_syll) if syll else None
        self.morph_tag_embedd = Embedding(num_morph_tags, morph_tag_dim) if morph_tag else None
        self.char_embedd = Embedding(num_chars, char_dim) if char else None
        self.pos_embedd = Embedding(num_pos, pos_dim, init_embedding=embedd_pos) if pos else None

        morph_num_filters = num_filters if morph_num_filters is None else morph_num_filters
        syll_num_filters = num_filters if syll_num_filters is None else syll_num_filters
        morph_tag_num_filters = num_filters if morph_tag_num_filters is None else morph_tag_num_filters
        char_num_filters = num_filters if char_num_filters is None else char_num_filters

        morph_kernel_size = kernel_size if morph_kernel_size is None else morph_kernel_size
        syll_kernel_size = kernel_size if syll_kernel_size is None else syll_kernel_size
        morph_tag_kernel_size = kernel_size if morph_tag_kernel_size is None else morph_tag_kernel_size
        char_kernel_size = kernel_size if char_kernel_size is None else char_kernel_size

        self.morph_conv1d = nn.Conv1d(morph_dim, morph_num_filters, morph_kernel_size, padding=morph_kernel_size - 1) if morph else None
        self.syll_conv1d = nn.Conv1d(syll_dim, syll_num_filters, syll_kernel_size, padding=syll_kernel_size - 1) if syll else None
        self.morph_tag_conv1d = nn.Conv1d(morph_tag_dim, morph_tag_num_filters, morph_tag_kernel_size, padding=morph_tag_kernel_size - 1) if morph_tag else None
        self.char_conv1d = nn.Conv1d(char_dim, char_num_filters, char_kernel_size, padding=char_kernel_size - 1) if char else None

        self.morph_cooccur = morph_cooccur
        self.morph_freq = morph_freq
        self.syll_cooccur = syll_cooccur
        self.syll_freq = syll_freq
        ...
        # num_features counts number of used features
        num_features = np.sum(np.array([morph, syll, morph_tag, char], dtype=np.int64))
        dim_enc += num_features * num_filters
    ...
    def _get_polysemous_embedding(self, inputs_row, feature_row, cooccur, freq):
        inputs_row = inputs_row.data.cpu().numpy()

        if np.count_nonzero(inputs_row - 1) == 0:
            return feature_row

        part_cooccur = cooccur[inputs_row][:, inputs_row].toarray().astype(np.float64)
        part_freq = (freq[0, inputs_row].T * freq[0, inputs_row]).toarray().astype(np.float64)

        W = part_cooccur / part_freq
        W[np.isnan(W)] = 0
        W[W >= 1] = 1 # regulation for upper bound in Embedding gradient

        W = torch.from_numpy(W)
        W = torch.autograd.Variable(W).float().cuda()

        feature_row = torch.matmul(W, feature_row) / np.count_nonzero(inputs_row - 1)
    ...
    def _get_feature_output(self, inputs, embedd, conv1d, cooccur=None, freq=None):
        # [batch, length, feature_length, feature_dim]
        feature = embedd(inputs)
        # TODO Debugging
        # print('feature point1:', feature[0, 0, 1, :10])

        feature_size = feature.size()

        # TODO: fine tune GloVe
        if cooccur is not None and freq is not None:
            inputs = inputs.view((-1, inputs.size(-1)))
            feature = feature.view((-1, feature.size(-2), feature.size(-1)))
            # print('feature point2:', feature[0, 1, :10]) # TODO Debuggingre_row

            # feature_row = torch.unbind(feature, dim=0)[0] # TODO Debugging
            # print('feature point3:', feature_row[1, :10])  # TODO Debugging

            tList = [self._get_polysemous_embedding(inputs_row, feature_row, cooccur, freq) + feature_row
                     for inputs_row, feature_row in zip(torch.unbind(inputs, dim=0), torch.unbind(feature, dim=0))]

            feature = torch.stack(tList, dim=0)
            feature = feature.view(feature_size)

            # TODO delete below

            # # [batch * length, morph_length]
            # inputs_reshaped = inputs.data.cpu().numpy()
            # inputs_reshaped = np.reshape(inputs_reshaped, (-1, inputs_reshaped.shape[-1]))
            #
            ...
            #         # [feature_length, feature_dim]
            #         C = np.squeeze(feature[batch_idx, sent_idx].data.cpu().numpy())
            #
            #         if np.count_nonzero(sent - 1) == 0:
            #             morph_features.append(C)
            #             continue
            #
            #         part_cooccur = cooccur[sent][:, sent].toarray().astype(np.float32) + 1e-4
            #         part_freq = (freq[0, sent].T * freq[0, sent]).toarray().astype(np.float32) + 1e-4
            #
            #         W = part_cooccur / part_freq
            #         morph_features.append(np.dot(W, C) / np.count_nonzero(sent - 1))
            #
            # feature = np.stack(morph_features)
            # feature = np.reshape(feature, (inputs.shape[0], inputs.shape[1], inputs.shape[2], -1))
            # feature = torch.from_numpy(feature)
            # feature = torch.autograd.Variable(feature).float()

        if conv1d is not None:
            # first transform to [batch *length, feature_length, feature_dim]
            # then transpose to [batch * length, feature_dim, feature_length]
            feature = feature.view(feature_size[0] * feature_size[1], feature_size[2], feature_size[3]).transpose(1, 2)
            # put into cnn [batch*length, feature_filters, feature_length]
            # then put into maxpooling [batch * length, feature_filters]
            feature, _ = conv1d(feature).max(dim=2)
            # reshape to [batch, length, feature_filters]
            feature = torch.tanh(feature).view(feature_size[0], feature_size[1], -1)
        else:
            raise ValueError('use_feature is True but no conv1d found')

        # apply dropout on input
        feature = self.dropout_in(feature)
        return feature

    def _get_encoder_output(self, input_word, input_morph, input_syll, input_morph_tag, input_char, input_pos, mask_e=None, length_e=None, hx=None):
        # [batch, length, word_dim]
        word = self.word_embedd(input_word)
        # apply dropout on input
        word = self.dropout_in(word)
        ...
        if self.morph:
            morph = self._get_feature_output(input_morph, self.morph_embedd, self.morph_conv1d, self.morph_cooccur, self.morph_freq)
            # concatenate word and morph [batch, length, word_dim+morph_filter]
            src_encoding = torch.cat([src_encoding, morph], dim=2)
            
        if self.syll:
            syll = self._get_feature_output(input_syll, self.syll_embedd, self.syll_conv1d, self.syll_cooccur, self.syll_freq)
            # concatenate word and syll [batch, length, word_dim+syll_filter]
            src_encoding = torch.cat([src_encoding, syll], dim=2)
            
        if self.morph_tag:
            morph_tag = self._get_feature_output(input_morph_tag, self.morph_tag_embedd, self.morph_tag_conv1d)
            # concatenate word and morph_tag [batch, length, word_dim+morph_tag_filter]
            src_encoding = torch.cat([src_encoding, morph_tag], dim=2)
            
        if self.char:
            char = self._get_feature_output(input_char, self.char_embedd, self.char_conv1d)
            # concatenate word and char [batch, length, word_dim+char_filter]
    ...
    def forward(self, input_word, input_morph, input_syll, input_morph_tag, input_char, input_pos, mask=None, length=None, hx=None):
        raise RuntimeError('Stack Pointer Network does not implement forward')

    def loss(self, input_word, input_morph, input_syll, input_morph_tag, input_char, input_pos, heads, stacked_heads, children, siblings, stacked_types, label_smooth,
        ...
        output_enc, hn, mask_e, _ = self._get_encoder_output(input_word, input_morph, input_syll, input_morph_tag, input_char, input_pos, mask_e=mask_e, length_e=length_e, hx=hx)
    ...
    def decode(self, input_word, input_morph, input_syll, input_morph_tag, input_char, input_pos, mask=None, length=None, hx=None, beam=1, leading_symbolic=0, ordered=True):
        ...
        output_enc, hn, mask, length = self._get_encoder_output(input_word, input_morph, input_syll, input_morph_tag, input_char, input_pos, mask_e=mask, length_e=length, hx=hx)
        # output size [batch, length_encoder, arc_space]
        arc_c = F.elu(self.arc_c(output_enc))
        # output size [batch, length_encoder, type_space]
        type_c = F.elu(self.type_c(output_enc))
